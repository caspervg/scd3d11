...
namespace nSCD3D11 {
    ...
    bool ConvertVertices(
        ...
        uint32_t const texCoordOffset = texCoordCount
                                            ? RZVertexFormatElementOffset(format, kGDElementType_TexCoord, 0)
                                            : 0;
        ...
        for (uint32_t index = 0; index < count; ++index, input += stride) {
            ...
            for (uint32_t coordinate = 0; coordinate < texCoordCount && coordinate < 2; ++coordinate) {
                memcpy(vertex.texCoord[coordinate], input + texCoordOffset + coordinate * sizeof(vertex.texCoord[0]),
                       sizeof(vertex.texCoord[0]));
            }
        }
        return true;
    }
    ...
    uint32_t D3D11MipLevelCount(uint32_t width, uint32_t height) {
        uint32_t levels = 1;
        for (uint32_t dimension = (std::max)(width, height); dimension > 1; dimension >>= 1) ++levels;
        return levels;
    }

    uint32_t D3D11MipDimension(uint32_t dimension, uint32_t level) {
        while (level-- > 0 && dimension > 1) dimension >>= 1;
    ...
    D3D11_BLEND D3D11AlphaBlend(uint32_t blend) {
        switch (D3D11Blend(blend)) {
            ...
            default: return D3D11Blend(blend);
        }
    }

    bool IsValidBlendFunction(uint32_t source, uint32_t destination) {
        return D3D11Blend(source) != 0 && D3D11Blend(destination) != 0 && destination != 10;
    }
